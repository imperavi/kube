...
{if(top.location.hash==='')return;if(!this.opts.scroll)return;if(this.$element.find('[rel='+top.location.hash+']').size()===0)return;this.open(top.location.hash);this.scrollTo(top.location.hash);},toggle:function(e)
...
if(value.length==1)return value[0];else return value;}
return(typeof data=='undefined')?e:data;},build:function()
{this.$dropdown=$(this.opts.target);this.$dropdown.hide();this.$caret=$('<b class="caret"></b>');this.$element.append(this.$caret);this.setCaretUp();this.preventBodyScroll();this.$element.click($.proxy(this.toggle,this));},setCaretUp:function()
...
this.$dropdown.css({position:position,top:top+'px',left:left+'px'});},show:function()
...
{this.$dropdown.on('mouseover',function(){$('html').css('overflow','hidden');});this.$dropdown.on('mouseout',function(){$('html').css('overflow','');});},hide:function(e)
{if(e)
...
if(value.length==1)return value[0];else return value;}
return(typeof data=='undefined')?e:data;},build:function()
{this.$sourceBox=$('<div class="filterbox" />');this.$sourceSelect=$('<span class="filterbox-toggle" />');this.$sourceLayer=$('<ul class="filterbox-list hide" />');this.$sourceResult=$('<select name="'+this.$element.attr('name')+'" id="'+this.$element.attr('id')+'" />').hide();this.$source=$('<input type="text" id="'+this.$element.attr('id')+'-input" class="'+this.$element.attr('class')+'" />');this.$sourceBox.append(this.$sourceResult);this.$sourceBox.append(this.$source);this.$sourceBox.append(this.$sourceSelect);this.$sourceBox.append(this.$sourceLayer);this.setPlaceholder();var key=this.$element.val();var value=this.$element.find('option:selected').text();this.setResult(key,value);this.$element.hide().after(this.$sourceBox);this.$element.find('option').each($.proxy(this.buildListItemsFromOptions,this));this.$element.removeAttr('id').removeAttr('name');this.$source.on('keyup',$.proxy(this.clearSelected,this));this.$sourceSelect.on('click',$.proxy(this.load,this));this.preventBodyScroll();},load:function(e)
...
{var value=this.$source.val();this.setResult(value,value);if(this.$source.val().length===0)this.$element.val(0);},setSelectedItem:function(items,value)
...
{e.preventDefault();var $el=$(item||e.target);var rel=$el.attr('rel');var text=$el.text();this.$source.val(text);this.$element.val(rel);this.close();this.setResult(rel,text);this.setCallback('select',{id:rel,value:text});},setResult:function(key,value)
{var option=$('<option value="'+key+'" selected="selected">'+value+'</option>');this.$sourceResult.html(option);},preventBodyScroll:function()
...
{if(!this.opts.placeholder&&!this.$element.attr('placeholder'))return;this.$source.attr('placeholder',(this.opts.placeholder)?this.opts.placeholder:this.$element.attr('placeholder'));},close:function(e)
...
{this.$element.addClass('navigation-toggle-show');this.$target.show();this.setCallback('show',this.$target);}
else
{this.$element.removeClass('navigation-toggle-show');this.$target.hide();this.setCallback('hide',this.$target);}},isTargetHide:function()
...
{e.preventDefault();var hash=$(e.target).attr('rel');this.show(hash);},readLocationHash:function(hash)
...
{$(this).hide();});this.links.parent().removeClass('active');},setEquals:function()
...
{return this.each(function()
{$.data(this,'textfit',{});$.data(this,'textfit',Textfit(this,options));});};function Textfit(el,options)
{return new Textfit.prototype.init(el,options);}
$.Textfit=Textfit;$.Textfit.NAME='textfit';$.Textfit.VERSION='1.0';$.Textfit.opts={min:'10px',max:'100px',compressor:1};Textfit.fn=$.Textfit.prototype={init:function(el,options)
{this.$element=el!==false?$(el):false;this.loadOptions(options);this.$element.css('font-size',Math.max(Math.min(this.$element.width()/(this.opts.compressor*10),parseFloat(this.opts.max)),parseFloat(this.opts.min)));},loadOptions:function(options)
{this.opts=$.extend({},$.extend(true,{},$.Textfit.opts),this.$element.data(),options);}};$(window).on('load.tools.textfit',function()
{$('[data-tools="textfit"]').textfit();});Textfit.prototype.init.prototype=Textfit.prototype;})(jQuery);(function($)
...
$.Upload=Upload;$.Upload.NAME='upload';$.Upload.VERSION='1.0';$.Upload.opts={url:false,placeholder:'Drop file here or ',param:false,module:false,padding:false,appendForm:false};Upload.fn=$.Upload.prototype={init:function(el,options)
...
{var param=(this.opts.param===false)?this.$element.attr('name'):this.opts.param;formData.append(param,file);if(this.opts.appendForm)
{var data=$(this.opts.appendForm).serializeArray();$.each(data,function(i,s)
{formData.append(s.name,s.value);});}
if(this.opts.module)
{formData.append('module',this.opts.module);}}
if($.progress)$.progress.show();this.setCallback('start');this.sendData(formData,e);},sendData:function(formData,e)
{var xhr=new XMLHttpRequest();xhr.open('POST',this.opts.url);xhr.onreadystatechange=$.proxy(function()
{if(xhr.readyState==4)
{var data=xhr.responseText;data=data.replace(/^\[/,'');data=data.replace(/\]$/,'');var json=(typeof data==='string'?$.parseJSON(data):data);if($.progress)$.progress.hide();this.$droparea.removeClass('drag-drop');this.setCallback('success',json);this.$element.val('');}},this);xhr.send(formData);},onDrag:function(e)
...
{$('[data-tools="upload"]').upload();});})(jQuery);